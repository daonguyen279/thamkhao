...
public class EmailUtils {
  ...
  public EmailUtils(String username, String password, String server, EmailFolder emailFolder) throws MessagingException {
    ...
    try {
      props.load(new FileInputStream(new File("src\\test\\java\\resource\\email.properties")));
    } catch(Exception e) {
      e.printStackTrace();
      System.exit(-1);
    }
  ...
  public static void connectMail() {
	  try {
	   emailUtils = new EmailUtils("myle.8174@gmail.com", "Infogroup1", "smtp.gmail.com", EmailUtils.EmailFolder.SPAM);
	  } catch (Exception e) {
	   e.printStackTrace();
	  }
	 }

	 public static void goToLinkWithinMail(WebDriver driver) {
	  try {
	   Message email = emailUtils.getLatestMessage();
	   String content = emailUtils.getMessageContent(email);
	//   String link = emailUtils.getUrlsFromMessage(email, "192.168.189.206:8888").get(0);
